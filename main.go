...
func main() {
	...
	collectTimeout := kingpin.Flag("collect-timeout", `Timeout for the entire collect operation`).Envar("PG_COLLECT_TIMEOUT").Default("5s").Duration()
	...
	c, err := collector.New(*dsn, *scrapeInterval, *collectTimeout, log)
	if err != nil {
		log.Error(err)
		return
	}
